...
describe('Address utils', () => {
  ...
  it('Convert from Hex address to bech32', () => {
    expect(AddressUtils.bech32Encode('000000000000000000010000000000000000000000000000000000000001ffff')).toStrictEqual('drt1qqqqqqqqqqqqqqqpqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqplllskzf8kp');
  });

  it('Convert from bech32 address to hex', () => {
    expect(AddressUtils.bech32Decode('drt1qqqqqqqqqqqqqqqpqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqplllskzf8kp')).toStrictEqual('000000000000000000010000000000000000000000000000000000000001ffff');
  });
  ...
  it('is smart contract address', () => {
    ...
    expect(AddressUtils.isSmartContractAddress('drt1qqqqqqqqqqqqqqqpqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqplllskzf8kp')).toStrictEqual(true);
  });
  ...
  it('compute shard for address', () => {
    ...
    expect(AddressUtils.computeShard(AddressUtils.bech32Decode('drt1qqqqqqqqqqqqqqqpqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqplllskzf8kp'), 3)).toEqual(4294967295);
  ...
  it('check if address is valid, return true', () => {
    expect(AddressUtils.isAddressValid('drt1qqqqqqqqqqqqqqqpqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqplllskzf8kp')).toStrictEqual(true);
  });

  it('check if address is not valid', () => {
    const address = AddressUtils.isAddressValid('drt1qqqqqqqqqqqqqqqpqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqplllskzf8kp');
    if (!address) {
